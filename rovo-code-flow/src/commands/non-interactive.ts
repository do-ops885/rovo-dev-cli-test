...
export async function nonInteractiveCommand(
  instruction: string,
  ...
  try {
    ...
    if (options.verbose) {
      console.log(
        chalk.yellow("Verbose mode enabled. Showing detailed output."),
      );
    }
    ...
    const result = await Promise.race([
      ...
      new Promise<boolean>((resolve) => {
        setTimeout(() => {
          console.log(
            chalk.red(`Instruction timed out after ${timeout / 1000} seconds.`),
          );
          resolve(false);
        }, timeout);
      }),
 ...
 */
export function saveInstructionResult(
  instruction: string,
  result: string,
): void {
  try {
