...
export async function memoryCommand(action: string, key?: string, value?: string, options: MemoryOptions = {}): Promise<void> {
  ...
  switch (action) {
    ...
    case 'store':
      if (!key) {
        console.log(chalk.red('Error: Note content is required for add action.'));
        return;
      }
    ...
    case 'delete':
      ...
      console.log(chalk.green(`Removing note from ${target} memory file matching: ${key}`));
      await memoryManager.removeNote(key, target);
      break;
      
    case 'show':
    case 'list':
      const memoryPath = target === 'global' 
    ...
    default:
      console.log(chalk.red(`Error: Unknown action '${action}'. Valid actions are: init, add/store, remove/delete, show/list`));
  }
}