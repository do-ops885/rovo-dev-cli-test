...
export async function mcpCommand(action: string, options: McpOptions = {}): Promise<void> {
  ...
  switch (action) {
    ...
    default:
      ...
      console.log(chalk.yellow('Available actions: start, stop, list, add, remove'));
  }
}
 ...
 * Start an MCP server
 */
async function startServer(mcpManager: McpManager, name: string): Promise<void> {
