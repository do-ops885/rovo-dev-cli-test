...
export async function feedbackCommand(
    ...
    feedbackDetails = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input.trim().length > 0 ? true : "Description is required",
      },
      {
        type: "input",
        name: "steps",
        ...
        validate: (input) =>
          input.trim().length > 0 ? true : "Steps are required",
      },
      {
        type: "input",
        name: "expected",
    ...
    feedbackDetails = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input.trim().length > 0 ? true : "Feedback is required",
      },
    ]);
  }
  ...
  const feedbackPath = path.join(
    feedbackDir,
    `feedback-${feedbackType}-${timestamp}.json`,
  );
