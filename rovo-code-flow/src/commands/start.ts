...
function startUi(port: string): void {
  try {
    ...
    const uiProcess = spawn('npx', ['--no', 'rovo-code-flow-ui', '--port', port], {
      stdio: 'pipe',
      detached: true
    });
    ...
    uiProcess.on('error', (error) => {
      ...
      console.log(chalk.yellow('Run "npm install -g rovo-code-flow-ui" to install UI dependencies.'));
    });
...
function displayCliHelp(): void {
  ...
  console.log('- rovo-code-flow event <role> <task>: Run an Event Modeling agent');
