...
export async function agentCommand(action: string, name?: string): Promise<void> {
  ...
  switch (action) {
    ...
    default:
      console.log(chalk.red(`Error: Unknown action '${action}'. Valid actions are: spawn, list, kill`));
  }
}
  ...
  switch (name.toLowerCase()) {
    ...
    default:
      ...
      if (name.toLowerCase().includes('code') || 
        ...
        console.log(chalk.yellow(`Unknown agent type: ${name}. Defaulting to Coder agent.`));
        agent = new CoderAgent();
      }
  }
