...
export async function toolsCommand(
  ...
  switch (action) {
    ...
    default:
      ...
      console.log(
        chalk.yellow("Available actions: list, allow, deny, reset, ask"),
      );
  }
}
 ...
 */
function allowTool(
  permissionsManager: ToolPermissionsManager,
  tool: string,
): void {
 ...
 */
async function allowToolInteractive(
  permissionsManager: ToolPermissionsManager,
): Promise<void> {
  const { tool, askEveryTime } = await inquirer.prompt([
    {
      ...
      validate: (input) =>
        input.trim().length > 0 ? true : "Tool name is required",
    },
    {
      type: "confirm",
      name: "askEveryTime",
 ...
 */
async function denyToolInteractive(
  permissionsManager: ToolPermissionsManager,
): Promise<void> {
  const { tool, askEveryTime } = await inquirer.prompt([
    {
 ...
 */
async function resetToolInteractive(
  permissionsManager: ToolPermissionsManager,
): Promise<void> {
  const permissions = permissionsManager.getAllPermissions();
  const toolNames = Object.keys(permissions);
...
function setToolToAsk(
  ...
  console.log(
    chalk.blue(`Tool "${tool}" will now ask for permission every time.`),
  );
}
 ...
 */
async function setToolToAskInteractive(
  permissionsManager: ToolPermissionsManager,
): Promise<void> {
  const { tool } = await inquirer.prompt([
    {
