...
export async function instructionsCommand(options: InstructionsOptions = {}): Promise<void> {
  const instructionsPath = path.join(os.homedir(), '.rovodev', 'instructions.yml');
...
async function listInstructions(): Promise<void> {
  ...
  try {
    ...
    console.log('Use "rovo-code-flow instructions --run <name>" to run an instruction.');
  } catch (error) {
...
async function addInstruction(): Promise<void> {
  const { name, prompt } = await inquirer.prompt([
    ...
    },
    {
      ...
      validate: (input) => input.trim().length > 0 ? true : 'Prompt is required'
    }
  ]);
  ...
  try {
    ...
    if (parsed.instructions.some((i: any) => i.name === name)) {
      console.log(chalk.yellow(`An instruction with the name "${name}" already exists.`));
      ...
      parsed.instructions = parsed.instructions.filter((i: any) => i.name !== name);
    }
...
async function removeInstruction(name: string): Promise<void> {
  ...
  try {
    ...
    const instructionIndex = parsed.instructions.findIndex((i: any) => i.name === name);
