...
import { Agent, TaskContext, TaskResult } from '../agents/agent.interface';
...
export async function swarmCommand(task: string, options: SwarmOptions): Promise<void> {
  ...
  try {
    ...
    if (agents.length === 0) {
      console.log(chalk.red('No agents created. Check the strategy or increase max agents.'));
      return;
    }
    ...
    const coordinationResult = await orchestrator.coordinateTask(task, agentNames, parallel);
    
    // Extract results
    const results: TaskResult[] = coordinationResult.results.map((r: any) => r.result);
 ...
 * Create agents based on strategy
 */
async function createAgentsForStrategy(strategy: string, maxAgents: number): Promise<Agent[]> {
  const agents: Agent[] = [];
