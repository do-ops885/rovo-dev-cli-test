...
export async function acliCommand(action: string, options: AcliOptions = {}): Promise<void> {
...
async function installRovoDev(acli: AcliIntegration): Promise<void> {
  ...
  if (!isAcliInstalled) {
    ...
    console.log('- macOS: https://developer.atlassian.com/platform/atlassian-cli/getting-started-with-acli/#install-acli-on-macos');
    console.log('- Linux: https://developer.atlassian.com/platform/atlassian-cli/getting-started-with-acli/#install-acli-on-linux');
    console.log('- Windows: https://developer.atlassian.com/platform/atlassian-cli/getting-started-with-acli/#install-acli-on-windows');
    return;
  }
  ...
  if (initialized) {
    ...
    console.log('1. Create an unscoped API token at https://id.atlassian.com/manage-profile/security/api-tokens');
    console.log('2. Run "rovo-code-flow acli auth" to authenticate with your Atlassian account');
  } else {
...
async function authRovoDev(acli: AcliIntegration): Promise<void> {
  ...
  console.log(chalk.yellow('Make sure you have created an unscoped API token at:'));
