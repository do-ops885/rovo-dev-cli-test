...
async function listSessions(): Promise<void> {
  ...
  sessions.forEach((session, index) => {
    ...
    console.log(`${activeMarker}${color(session.id)} - ${session.title} (${date}, ${session.messages} messages)`);
  });
...
async function clearSession(): Promise<void> {
  ...
  try {
    ...
    const sessionData = JSON.parse(fs.readFileSync(currentSession.file, 'utf8'));
    ...
    console.log(chalk.green(`Session "${currentSession.details.title}" has been cleared.`));
  } catch (error) {
...
async function pruneSession(): Promise<void> {
  ...
  try {
    ...
    const prunedMiddleMessages = sessionData.messages.slice(1, -3).map(msg => ({
      role: msg.role,
    ...
    sessionData.messages = [firstMessage, ...prunedMiddleMessages, ...lastMessages];
    
    // Update the file
    fs.writeFileSync(currentSession.file, JSON.stringify(sessionData, null, 2));
    
    console.log(chalk.green(`Session "${currentSession.details.title}" has been pruned to reduce token usage.`));
  } catch (error) {
...
async function switchSession(sessionId: string): Promise<void> {
  ...
  const targetSessionFile = sessionFiles.find(file => path.basename(file, '.json') === sessionId);
  ...
  try {
    ...
    console.log(chalk.green(`Switched to session "${sessionDetails.title}" (${sessionId}).`));
    
    // Display message count
    console.log(chalk.blue(`This session has ${sessionDetails.messages} messages.`));
  } catch (error) {
