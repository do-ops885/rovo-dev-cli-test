  ...
  switch (mainCommand) {
    ...
    case 'memory':
      if (parts[1] === 'init') {
        ...
        console.log(chalk.yellow('Use # to add notes to memory or #! to remove notes.'));
        console.log(chalk.yellow('Use /memory init to initialize memory with repository information.'));
      }
      break;
...
function showHelp(): void {
  ...
  console.log('/prune: Reduce the token size of the current session\'s message history while retaining context.');
  console.log('/instructions: Run saved instructions.');
  console.log('/memory init: Initialize memory file with repository information.');
